...
<head>
    ...
    <title>Payment - <?php echo e($paymentLink->title); ?> - Ipay</title>
        ...
        <div class="payment-card">
            ...
            <div class="left-panel">
                ...
                <div class="amount-section">
                    ...
                    <?php if($paymentLink->allow_partial_payment): ?>
                        <?php if(($paymentLink->amount_paid ?? 0) > 0): ?>
                            <div style="margin-top: 12px; padding-top: 12px; border-top: 1px solid rgba(255, 255, 255, 0.2);">
                                <div style="font-size: 12px; opacity: 0.9; margin-bottom: 6px;">Amount Paid</div>
                                <div style="font-size: 18px; font-weight: 600; color: #10b981;"><?php echo e($paymentLink->currency); ?> <?php echo e(number_format($paymentLink->amount_paid ?? 0, 2)); ?></div>
                                <div style="font-size: 12px; opacity: 0.9; margin-top: 8px; margin-bottom: 6px;">Remaining Balance</div>
                                <div style="font-size: 18px; font-weight: 600; color: #fbbf24;"><?php echo e($paymentLink->currency); ?> <?php echo e(number_format($paymentLink->getRemainingBalance(), 2)); ?></div>
                            </div>
                        <?php else: ?>
                            <div style="margin-top: 10px; padding: 10px; background: rgba(251, 191, 36, 0.15); border-radius: 8px; border: 1px solid rgba(251, 191, 36, 0.3);">
                                ...
                                <div style="font-size: 13px; opacity: 0.9;">
                                    Pay any amount up to <?php echo e($paymentLink->currency); ?> <?php echo e(number_format($paymentLink->amount, 2)); ?>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-12">
                            ...
                            <div class="input-group" style="margin-bottom: 10px;">
                                <span class="input-group-text" style="background: white; color: #667eea; font-weight: 700; font-size: 16px; border: none;"><?php echo e($paymentLink->currency); ?></span>
                                <input type="number" class="form-control form-control-lg" id="customAmount" 
                                       placeholder="Enter amount (max: <?php echo e(number_format($paymentLink->amount, 2)); ?>)" 
                                       min="0.01" 
                                       max="<?php echo e($paymentLink->amount); ?>" 
                                       step="0.01"
                                       value="<?php echo e($paymentLink->getRemainingBalance()); ?>"
                                       required
                            ...
                            <small class="d-block mt-2" style="font-size: 13px; color: rgba(255, 255, 255, 0.9);">
                                <?php if(($paymentLink->amount_paid ?? 0) > 0): ?>
                                    <strong>✓ Already paid:</strong> <?php echo e($paymentLink->currency); ?> <?php echo e(number_format($paymentLink->amount_paid, 2)); ?> | 
                                    <strong>Remaining:</strong> <?php echo e($paymentLink->currency); ?> <?php echo e(number_format($paymentLink->getRemainingBalance(), 2)); ?>

                                <?php else: ?>
                                    <strong>Total amount:</strong> <?php echo e($paymentLink->currency); ?> <?php echo e(number_format($paymentLink->amount, 2)); ?>. 
    ...
    <script>
        ...
        function initializeCashFreeCheckout(result) {
            try {
                ...
                cashfree.checkout(checkoutOptions).then(function(checkoutResult) {
                    ...
                    payButton.dataset.processing = '';
                    <?php if($paymentLink->allow_partial_payment): ?>
                    const remainingBalance = parseFloat(<?php echo e($paymentLink->getRemainingBalance()); ?>);
        ...
        async function verifyCashFreePayment(gatewayOrderId, transactionId) {
            try {
                ...
                const verifyResponse = await fetch(`/pay/<?php echo e($paymentLink->link_token); ?>/verify-cashfree`, {
                    method: 'POST',
                    headers: {
        ...
        if (customAmountInput && amountErrorDiv) {
            const remainingBalanceValue = <?php echo e($paymentLink->getRemainingBalance()); ?>;
            const totalAmountValue = <?php echo e($paymentLink->amount); ?>;
        ...
        const paymentLinkAmount = <?php echo e($paymentLink->amount); ?>;
        const paymentLinkCurrency = '<?php echo e($paymentLink->currency); ?>';
        <?php if($paymentLink->allow_partial_payment): ?>
        const paymentLinkRemainingBalance = <?php echo e($paymentLink->getRemainingBalance()); ?>;
        <?php endif; ?>
        ...
        payButton.addEventListener('click', async (event) => {
            ...
            if (customAmountInput && customAmountInput.value) {
                ...
                const totalAmount = parseFloat(<?php echo e($paymentLink->amount); ?>);
                const remainingBalance = parseFloat(<?php echo e($paymentLink->getRemainingBalance()); ?>);
                ...
                if (result.success) {
                    ...
                    if (result.gateway === 'razorpay' && result.use_razorpay_checkout && result.razorpay_key && result.razorpay_order_id) {
                        ...
                        const options = {
                            ...
                            name: '<?php echo e($paymentLink->merchant->name); ?>',
                            description: '<?php echo e($paymentLink->title); ?>',
                            order_id: result.razorpay_order_id,
                            prefill: {
...
<?php /**PATH C:\Users\dell\Downloads\Ipay-pg\resources\views/checkout/payment.blade.php ENDPATH**/ ?>