/** Ipay Payment Gateway SDK v2.0 **/
(function(window){
  'use strict';
  
  var Ipay = window.Ipay || {};
  var API_BASE_URL = window.IPAY_API_URL || window.location.origin;

  /**
   * Ipay Checkout Widget
   * Usage:
   ...
   *   checkout.open();
   */
  Ipay.Checkout = function(options) {
    this.options = options || {};
    this.validate();
    this.mode = this.detectMode(this.options.key);
  };

  Ipay.Checkout.prototype.validate = function() {
  ...
  Ipay.Checkout.prototype.detectMode = function(key) {
    if (!key) return 'test';
    return key.startsWith('pk_live_') ? 'live' : 'test';
  };

  Ipay.Checkout.prototype.open = function() {
    var self = this;
    var overlay = document.createElement('div');
    overlay.id = 'ipay-overlay';
    ...
    // Add CSS animations
    if (!document.getElementById('ipay-styles')) {
      var style = document.createElement('style');
      style.id = 'ipay-styles';
  ...
  Ipay.Checkout.prototype.close = function() {
    var overlay = document.getElementById('ipay-overlay');
    if (overlay) {
  ...
  Ipay.Checkout.prototype.buildCheckoutUrl = function() {
   ...
   */
  Ipay.createPaymentLink = function(options, callback) {
  ...
  // Constructor with .open() for integration samples: new Ipay({ key, mode }); ipay.open({ amount, ... });
  function IpayWidget(options) {
    this.key = options && options.key;
    this.mode = (options && options.mode) || 'test';
  }
  IpayWidget.prototype.open = function(paymentOptions) {
    var opts = Object.assign({}, paymentOptions, { key: this.key, mode: this.mode });
    var checkout = new Ipay.Checkout(opts);
    checkout.open();
  };
  window.Ipay = IpayWidget;
  window.Ipay.Checkout = Ipay.Checkout;
  window.Ipay.createPaymentLink = Ipay.createPaymentLink;
})(window);
