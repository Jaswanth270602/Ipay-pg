...
(function() {
    ...
    app.controller('PaymentLinksController', ['$http', '$window', function($http, $window){
        ...
        vm.newLink={ title:'', description:'', amount:'', currency:'USD', expires_in_hours:24 };
        ...
        function createPaymentLink(){ vm.creating=true; var csrf=document.querySelector('meta[name="csrf-token"]').content; var pk=(typeof window.ipayPaymentLinkPublicKey==='string'&&window.ipayPaymentLinkPublicKey.length)?window.ipayPaymentLinkPublicKey:''; if(!pk){ vm.creating=false; showToast('No API key for your current mode. Configure API Keys first.','error'); return; } $http.post('/merchant/payment-links', vm.newLink, {headers:{'X-CSRF-TOKEN':csrf,'X-API-KEY':pk}}).then(function(){ vm.creating=false; showToast('Payment link created','success'); var m=bootstrap.Modal.getInstance(document.getElementById('createLinkModal')); if(m) m.hide(); vm.newLink={ title:'', description:'', amount:'', currency:'USD', expires_in_hours:24 }; loadPaymentLinks(); }, function(){ vm.creating=false; showToast('Failed to create link','error'); }); }
