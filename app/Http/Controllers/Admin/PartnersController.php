...
class PartnersController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $validator = Validator::make($normalize, [
            ...
            'team_name' => ['nullable', 'string', 'max:255', 'regex:/^[A-Za-z ]+$/'],
            'team_type' => 'nullable|string|max:255',
            'phone' => ['required', 'regex:/^\+?[1-9]\d{6,14}$/', 'unique:partners,phone'],
            'email' => 'required|email|max:255|unique:partners,email',
            'is_approved' => 'required|boolean',
            'is_internal' => 'required|boolean',
            'referral_code' => ['nullable', 'string', 'max:32', 'regex:/^[A-Z0-9]+$/', 'unique:partners,referral_code'],
            'whitelabel_url' => 'nullable|url|max:500',
            'registration_date' => 'nullable|date_format:d-m-Y',
            'ref' => ['nullable', 'string', 'max:100', 'regex:/^[A-Za-z ]+$/'],
            ...
            'phone.regex' => 'Mobile must be 7-15 digits with optional leading +.',
            ...
            'referral_code.max' => 'Referral code may not be greater than 32 characters.',
            'referral_code.regex' => 'Referral code may contain only letters and numbers.',
            ...
            'ref.regex' => 'Ref may contain only letters and spaces.',
    ...
    public function update(Request $request, $id): JsonResponse
    {
        ...
        $validator = Validator::make($request->all(), [
            'name' => 'sometimes|required|string|min:3|max:256',
            'user_name' => 'nullable|string|max:255',
            'team_name' => ['nullable', 'string', 'max:255', 'regex:/^[A-Za-z ]+$/'],
            'team_type' => 'nullable|string|max:255',
            'organization_name' => 'nullable|string|max:255',
            'phone' => ['sometimes', 'required', 'regex:/^\+?[1-9]\d{6,14}$/', 'max:16'],
            'email' => 'sometimes|required|email|max:255|unique:partners,email,' . $id,
            'is_approved' => 'boolean',
            'is_internal' => 'boolean',
            'referral_code' => ['nullable', 'string', 'max:32', 'regex:/^[A-Z0-9]+$/', 'unique:partners,referral_code,' . $id],
            'whitelabel_url' => 'nullable|url|max:500',
