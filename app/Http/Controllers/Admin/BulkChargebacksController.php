...
class BulkChargebacksController extends Controller
{
    ...
    {
        $this->logInfo('Admin bulk chargebacks page accessed', ['user_id' => auth()->id()]);

        return view('admin.payments.bulk-chargebacks');
    }
    ...
    public function getJobs(Request $request): JsonResponse
    {
        try {
            ...
            $query = DB::table('bulk_chargeback_jobs')->latest();
            if ($isTestMode) {
    ...
    public function downloadTemplate(): \Symfony\Component\HttpFoundation\BinaryFileResponse
    {
        $relativePath = $this->fileLifecycleService->createCsvReport('bulk_chargeback_template.csv', function ($file): void {
            ...
            fputcsv($file, ['CB_SAMPLE_001', '1', 'TXN_SAMPLE_001', '100.00', 'pending']);
        });
    ...
    public function downloadStatusFile(int $id): \Symfony\Component\HttpFoundation\BinaryFileResponse
    {
        $adminViewMode = session('admin_view_mode', 'test');
        $isTestMode = $adminViewMode === 'test';
        $jobQuery = DB::table('bulk_chargeback_jobs')->where('id', $id);
        if ($isTestMode) {
